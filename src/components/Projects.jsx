...
const projects = [
  {
    ...
    points: [
      ...
      'Implemented a custom "line-crossing" algorithm to accurately count and classify vehicles entering specific zones.',
      'Deployed an interactive Streamlit dashboard allowing users to upload video feeds and visualize live traffic analytics.',
      'Optimized inference performance to achieve real-time frame rates on standard CPU hardware without GPU acceleration.',
    ],
  ...
  },
  {
    ...
    points: [
      ...
      'Designed a normalized PostgreSQL database schema to efficiently handle complex relationships between users, equipment, and rentals.',
      'Implemented secure booking workflows and an admin dashboard featuring inventory management and role-based access control (RBAC).',
      'Utilized modern React hooks and Context API for global state management to ensure a seamless user experience.',
    ],
  ...
  },
  {
    ...
    points: [
      'Developed a hybrid recommendation engine combining Content-Based filtering (TF-IDF) and Collaborative Filtering to maximize accuracy.',
      'Utilized Cosine Similarity metrics to map user preferences against a large-scale movie dataset.',
      'Addressed the "cold-start" problem for new users by implementing popularity-based fallback mechanisms.',
      'Deployed the model via a responsive Streamlit web interface allowing real-time movie search and suggestions.',
    ],
  ...
  },
  {
    ...
    points: [
      'Implemented a Split Federated Learning architecture (IEEE ICDCS 2023) to enable privacy-preserving distributed training.',
      'Simulated client-server interactions using PyTorch, optimizing computational load by splitting model layers across edge devices.',
      'Engineered a synchronized forward/backward propagation pipeline to train neural networks across distributed nodes.',
      'Conducted comparative analysis against traditional Federated Learning, demonstrating reduced client-side computational overhead.',
    ],
  ...
  },
  {
    ...
    points: [
      'Built a Recurrent Neural Network (RNN) using TensorFlow to analyze temporal employee behavioral patterns.',
      'Achieved 80% prediction accuracy through rigorous data preprocessing, feature engineering, and model hyperparameter tuning.',
      'Implemented techniques to handle class imbalance, ensuring robust predictions for both churn and retention cases.',
      'Visualized model performance using Matplotlib to plot loss and accuracy curves across training epochs.',
    ],
  ...
  },
  {
    ...
    points: [
      'Developed a high-performance REST API using FastAPI with Pydantic models to handle vehicle listings and transactions.',
      'Integrated NeonDB (serverless PostgreSQL) for storage and implemented secure JWT authentication for user sessions.',
      'Built a dynamic Streamlit frontend enabling users to filter cars by price, make, and year with real-time updates.',
      'Leveraged asynchronous programming in Python to handle multiple concurrent database requests efficiently.',
    ],
